...
module.exports = async (pluginConfig, context) => {
    ...
    if (pluginConfig.registry) {
        ...
        await publishChartToRegistry(pluginConfig.chartPath, pluginConfig.registry, chart.name, chart.version, pluginConfig.updateDependencies);
...
async function updateDependencies(configPath, registry) {
    dependencies = await getDependencies(configPath)
    for (const dependency of dependencies) {
        latestVersion = await getLatestDependencyVersion(registry, dependency.name)
        dependency.version = latestVersion
    }
    const filePath = path.join(chartPath, 'Chart.yaml');
    const chartYaml = await fsPromises.readFile(filePath);
    chart = yaml.load(chartYaml);
    chart.dependencies = dependencies
    fs.writeFileSync(configPath+"/Chart.yaml", yaml.dump(chart));
}

async function getDependencies(chartPath) {
    const filePath = path.join(chartPath, 'Chart.yaml');
    const chartYaml = await fsPromises.readFile(filePath);
    const chart = yaml.load(chartYaml);
...
async function getLatestDependencyVersion(registry, name) {
    const {stdout} = await execa(
        'helm',
        ['show', 'chart', name, registry+"/"+name],
    );

    chart = yaml.load(stdout);
    return chart.version
}

async function publishChartToRegistry(configPath, registry, name, version, updateDeps) {
    if (updateDeps) {
        await updateDependencies(configPath, registry, name)
    }
